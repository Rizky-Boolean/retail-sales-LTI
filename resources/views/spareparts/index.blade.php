            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6 md:p-8 border border-gray-200 dark:border-gray-700">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    ...
                    <div class="flex justify-end">
                        <a href="{{ route('spareparts.create') }}" class="inline-flex items-center justify-center px-6 py-2.5 border border-transparent text-base font-medium rounded-lg shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition duration-150 ease-in-out">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                            </svg>
                            Tambah Sparepart
                        </a>
                        <a href="{{ route('spareparts.import.show') }}" class="inline-flex items-center justify-center px-6 py-2.5 border border-transparent text-base font-medium rounded-lg shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition duration-150 ease-in-out ml-2">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                            </svg>
                            Import Data
                        </a>
                        <a href="{{ route('spareparts.trash') }}" class="inline-flex items-center justify-center px-6 py-2.5 border border-transparent text-base font-medium rounded-lg shadow-sm text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition duration-150 ease-in-out ml-2">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                    <table id="sparepartsTable" class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-200 dark:bg-gray-700">
                            <tr>
                                <th class="text-left py-3 px-4 uppercase font-semibold text-sm text-gray-700 dark:text-gray-300">Kode Part</th>
                                <th class="text-left py-3 px-4 uppercase font-semibold text-sm text-gray-700 dark:text-gray-300">Nama Part</th>
                                <th class="text-left py-3 px-4 uppercase font-semibold text-sm text-gray-700 dark:text-gray-300">Satuan</th>
                                <th class="text-left py-3 px-4 uppercase font-semibold text-sm text-gray-700 dark:text-gray-300">Harga Jual</th>
                        ...
                        <tbody class="text-gray-700 dark:text-gray-300">
                            @forelse($spareparts as $sparepart)
                                <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                    ...
                                    <td class="py-3 px-4">{{ $sparepart->satuan }}</td>
                                    <td class="py-3 px-4 text-left">{{ 'Rp ' . number_format($sparepart->harga_jual, 0, ',', '.') }}</td>
    ...
    <script>
        function filterTable() {
            ...
            for (let i = 0; i < tr.length; i++) {
                ...
                if (tdKode && tdKode.textContent.toUpperCase().indexOf(filter) > -1) found = true;
                if (tdNama && tdNama.textContent.toUpperCase().indexOf(filter) > -1) found = true;
        ...
        function showDeleteModal(id) {
            document.getElementById('deleteForm').action = `/spareparts/${id}`;
