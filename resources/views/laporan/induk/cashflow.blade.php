        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6 md:p-8 border border-gray-200 dark:border-gray-700">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center mb-6">
                    <a href="{{ route('laporan.induk.index') }}" class="inline-flex items-center text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200 font-medium mb-4 md:mb-0">
                ...
                {{-- Filter Form --}}
                <form action="{{ route('laporan.induk.index') }}" method="GET"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                    <div class="bg-green-50 dark:bg-gray-800/50 p-6 rounded-lg shadow-md border border-green-100 dark:border-gray-700 flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p class="text-3xl font-bold text-green-800 dark:text-green-200">{{ 'Rp ' . number_format($totalPemasukan ?? 0, 0, ',', '.') }}</p>
                        </div>
                    </div>
                    <div class="bg-red-50 dark:bg-gray-800/50 p-6 rounded-lg shadow-md border border-red-100 dark:border-gray-700 flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p class="text-3xl font-bold text-red-800 dark:text-red-200">{{ 'Rp ' . number_format($totalPengeluaran ?? 0, 0, ',', '.') }}</p>
                        </div>
                    </div>
                    <div class="bg-blue-50 dark:bg-gray-800/50 p-6 rounded-lg shadow-md border border-blue-100 dark:border-gray-700 flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p class="text-3xl font-bold text-blue-800 dark:text-blue-200">{{ 'Rp ' . number_format($arusKasBersih ?? 0, 0, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    {{-- Pemasukan --}}
                    <div>
                        <h4 class="text-lg font-bold mb-4 text-gray-800 dark:text-gray-200">Rincian Pemasukan (Penjualan Cabang)</h4>
                            ...
                            <table id="laporan-pemasukan-table" class="min-w-full text-sm divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-200 dark:bg-gray-700">
                                    <tr>
                                        ...
                                        <th class="py-3 px-4 text-left uppercase font-semibold text-xs text-gray-800 dark:text-gray-500 tracking-wider">ID Penjualan</th>
                                        <th class="py-3 px-4 text-left uppercase font-semibold text-xs text-gray-800 dark:text-gray-500 tracking-wider">Cabang</th>
                                ...
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700 text-gray-700 dark:text-gray-300">
                                    ...
                                    <tr>
                                        <td class="py-3 px-4 whitespace-nowrap">{{ \Carbon\Carbon::parse($item->tanggal_penjualan)->format('d/m/Y') }}</td>
                                        <td class="py-3 px-4 whitespace-nowrap font-mono">#SALE-{{ $item->id }}</td>
                                        <td class="py-3 px-4 whitespace-nowrap">{{ $item->cabang->nama_cabang ?? 'N/A' }}</td>
                                        <td class="py-3 px-4 text-right whitespace-nowrap font-semibold">Rp {{ number_format($item->total_final, 0, ',', '.') }}</td>
                                    </tr>
                                    @empty
                                    <tr>
                                        <td colspan="4" class="text-center p-4 text-gray-500 dark:text-gray-400">Tidak ada pemasukan.</td>
                                    </tr>
                                    @endforelse
                                </tbody>
                                <tfoot class="bg-gray-100 dark:bg-gray-900/50 font-bold">
                                    <tr>
                                        <td colspan="3" class="py-3 px-4 text-right">Total Pemasukan:</td>
                                        <td class="py-3 px-4 text-right">{{ 'Rp ' . number_format($totalPemasukan ?? 0, 0, ',', '.') }}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                    ...
                    {{-- Pengeluaran --}}
                    <div>
                        <h4 class="text-lg font-bold mb-4 text-gray-800 dark:text-gray-200">Rincian Pengeluaran (Pembelian Stok)</h4>
                        <div class="overflow-x-auto rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
                            <table id="laporan-pengeluaran-table" class="min-w-full text-sm divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-200 dark:bg-gray-700">
                                    <tr>
                                        <th class="py-3 px-4 text-left uppercase font-semibold text-xs text-gray-800 dark:text-gray-500 tracking-wider">Tanggal</th>
                                        <th class="py-3 px-4 text-left uppercase font-semibold text-xs text-gray-800 dark:text-gray-500 tracking-wider">ID Stok Masuk</th>
                                        <th class="py-3 px-4 text-left uppercase font-semibold text-xs text-gray-800 dark:text-gray-500 tracking-wider">Supplier</th>
                                        <th class="py-3 px-4 text-right uppercase font-semibold text-xs text-gray-800 dark:text-gray-500 tracking-wider">Jumlah</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700 text-gray-700 dark:text-gray-300">
                                    ...
                                    <tr>
                                        <td class="py-3 px-4 whitespace-nowrap">{{ \Carbon\Carbon::parse($item->tanggal_masuk)->format('d/m/Y') }}</td>
                                        <td class="py-3 px-4 whitespace-nowrap font-mono">#IN-{{ $item->id }}</td>
                                        <td class="py-3 px-4 whitespace-nowrap">{{ $item->supplier->nama_supplier ?? 'N/A' }}</td>
                                        <td class="py-3 px-4 text-right whitespace-nowrap font-semibold">Rp {{ number_format($item->total_final, 0, ',', '.') }}</td>
                                    </tr>
                                    @empty
                                    <tr>
                                        <td colspan="4" class="text-center p-4 text-gray-500 dark:text-gray-400">Tidak ada pengeluaran.</td>
                                    </tr>
                                    @endforelse
                                </tbody>
                                <tfoot class="bg-gray-100 dark:bg-gray-900/50 font-bold">
                                    <tr>
                                        <td colspan="3" class="py-3 px-4 text-right">Total Pengeluaran:</td>
                                        <td class="py-3 px-4 text-right">{{ 'Rp ' . number_format($totalPengeluaran ?? 0, 0, ',', '.') }}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
...
document.getElementById('export-pdf-btn').addEventListener('click', function () {
    ...
    const tanggalAwal = "{{ \Carbon\Carbon::parse($tanggalAwal ?? now())->format('d M Y') }}";
    const tanggalAkhir = "{{ \Carbon\Carbon::parse($tanggalAkhir ?? now())->format('d M Y') }}";
    const namaGudang = "Gudang Induk";
    const totalPemasukan = "{{ 'Rp ' . number_format($totalPemasukan ?? 0, 0, ',', '.') }}";
    const totalPengeluaran = "{{ 'Rp ' . number_format($totalPengeluaran ?? 0, 0, ',', '.') }}";
    const arusKasBersih = "{{ 'Rp ' . number_format($arusKasBersih ?? 0, 0, ',', '.') }}";
    const isArusKasNegatif = {{ ($arusKasBersih ?? 0) < 0 ? 'true' : 'false' }};
    ...
    const addHeaderFooter = (data) => {
        ...
        doc.text('Laporan Arus Kas (Cashflow) Induk', pageWidth / 2, 40, { align: 'center' });
        doc.setFontSize(10);
        doc.setFont(undefined, 'normal');
        doc.text(`Gudang: ${namaGudang}`, pageWidth / 2, 55, { align: 'center' });
    ...
    doc.save(`Laporan-Cashflow-Induk-${tanggalAwal}.pdf`);
});
</script>