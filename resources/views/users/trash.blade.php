<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="mb-4">
                        <a href="{{ route('users.index') }}" class="inline-flex items-center px-4 py-2 text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200 font-semibold rounded-lg transition duration-150 ease-in-out">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                            {{ __('Kembali ke Manajemen Pengguna') }}
                        </a>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full bg-white dark:bg-gray-800">
                            <thead class="bg-gray-200 dark:bg-gray-700">
                                <tr>
                                    <th class="py-3 px-4 text-left uppercase font-semibold text-xs text-gray-800 dark:text-gray-500 tracking-wider">Nama Pengguna</th>
                                    <th class="py-3 px-4 text-left uppercase font-semibold text-xs text-gray-800 dark:text-gray-500 tracking-wider">Email</th>
                            ...
                            <tbody>
                                @forelse($users as $user)
                                    <tr class="border-b border-gray-200 dark:border-gray-700">
                                        ...
                                        <td class="py-3 px-4">{{ $user->deleted_at->format('d M Y, H:i') }}</td>
                                        <td class="py-3 px-4 text-center">
                                            ...
                                            <div class="flex justify-center items-center gap-4">
                                                ...
                                                <button type="button" onclick="showDeleteModal('{{ route('users.forceDelete', $user->id) }}', '{{ addslashes($user->name) }}')" class="flex items-center gap-1 text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300" title="Hapus Permanen">
                                ...
                                @empty
                                    <tr><td colspan="4" class="text-center py-4 text-gray-500 dark:text-gray-400">Tidak ada data di dalam trash.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
    ...
    {{-- Modal Konfirmasi Hapus (Desain Baru) --}}
    <div id="deleteModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-sm mx-auto">
            <div class="text-center">
                ...
                <h3 id="deleteModalTitle" class="mb-5 text-lg font-normal text-gray-800 dark:text-gray-300"></h3>
    ...
    {{-- Script untuk Modal Dinamis (Desain Baru) --}}
    <script>
